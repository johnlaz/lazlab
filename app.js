...
// ── ZIP INSPECTOR ────────────────────────────────────
window.LazNote.ingestZIP = async function() {
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      toast(`✓ Loaded ${files.length} files`, 'lime');
      nav('zip');
      renderZIPInspector();
    } catch (err) {
      toast('Failed to read ZIP: ' + err.message, 'red');
...
window.LazNote.renderZIPInspector = function() {
  renderZIPInspector();
};

function renderZIPInspector() {
  const session = state.zipSession;
  const body = document.getElementById('zip-body');
  if (!session) {
    body.innerHTML = `
      <button class="btn primary block" onclick="LazNote.ingestZIP()">📦 Upload ZIP</button>
      <div style="font-size:12px;color:var(--ink-50);text-align:center;padding:20px 0;">Select a ZIP file to explore its contents, preview files, and analyze with AI.</div>
    `;
    document.getElementById('zip-title').textContent = 'Upload ZIP';
    document.getElementById('zip-export-btn').style.display = 'none';
    return;
  }
  
  document.getElementById('zip-title').textContent = session.name;
  document.getElementById('zip-export-btn').style.display = 'block';
  ...
  let html = `
    <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap;">
      <div style="flex:1;min-width:150px;background:var(--line-2);padding:12px;border-radius:8px;text-align:center;">
        <div style="font-size:24px;font-weight:700;color:var(--c9a227);">${session.fileCount}</div>
        <div style="font-size:11px;color:var(--ink-50);margin-top:2px;">Total Files</div>
      </div>
      <div style="flex:1;min-width:150px;background:var(--line-2);padding:12px;border-radius:8px;text-align:center;">
        <div style="font-size:24px;font-weight:700;color:var(--lime);">${sortedTypes.length}</div>
        <div style="font-size:11px;color:var(--ink-50);margin-top:2px;">File Types</div>
      </div>
    </div>
    <div style="font-size:11px;font-weight:600;color:var(--ink-60);margin-bottom:8px;text-transform:uppercase;letter-spacing:0.05em;">By Type</div>
  `;
  
  sortedTypes.forEach(type => {
    const count = grouped[type].length;
    html += `
      <div style="padding:10px 12px;background:var(--line-2);border-radius:6px;margin-bottom:6px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;" onclick="LazNote.expandFileType('${type}')">
        <div style="display:flex;align-items:center;gap:8px;">
          <span style="font-family:var(--mono);font-size:10px;font-weight:600;color:var(--c9a227);">.${type}</span>
          <span style="font-size:12px;flex:1;">${getTypeLabel(type)}</span>
        </div>
        <span style="font-size:11px;color:var(--ink-50);">${count}</span>
      </div>
    `;
  });
  ...
  html += `
    <div style="margin-top:16px;display:flex;gap:8px;">
      <button class="btn block" style="flex:1;" onclick="LazNote.askGroqAboutZIP()">🤖 Ask AI</button>
      <button class="btn block" style="flex:1;" onclick="LazNote.exportZIPInventory()">📋 Inventory</button>
    </div>
  `;
...
window.LazNote.expandFileType = async function(type) {
  const session = state.zipSession;
  if (!session) return;
  
  const files = session.files.filter(f => f.type === type);
  const body = document.getElementById('zip-body');
  
  let html = `
    <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;">
      <button class="icon-btn" onclick="LazNote.renderZIPInspector()" style="font-size:14px;">←</button>
      <div style="flex:1;font-weight:600;">Files (.${type}) — ${files.length}</div>
    </div>
  `;
  ...
  files.forEach(f => {
    ...
    html += `
      <div style="padding:10px 12px;background:var(--line-2);border-radius:6px;margin-bottom:6px;cursor:pointer;" onclick="LazNote.previewFile('${f.path.replace(/'/g, "\\'")}')">
        <div style="font-family:var(--mono);font-size:11px;color:var(--lime);word-break:break-all;margin-bottom:4px;">${escapeHtml(f.path)}</div>
        <div style="font-size:10px;color:var(--ink-50);">${sizeKB} KB</div>
      </div>
    `;
  });
...
window.LazNote.previewFile = async function(path) {
  const session = state.zipSession;
  if (!session || !session.zip) return;
  ...
  try {
    ...
    const content = await session.zip.file(path).async('string');
    const preview = content.slice(0, 2000); // First 2000 chars
    const body = document.getElementById('zip-body');
    
    let html = `
      <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;">
        <button class="icon-btn" onclick="LazNote.expandFileType('${file.type}')" style="font-size:14px;">←</button>
        <div style="flex:1;font-family:var(--mono);font-size:11px;color:var(--c9a227);word-break:break-all;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(path)}</div>
      </div>
      <div style="background:var(--line-2);padding:12px;border-radius:6px;font-family:var(--mono);font-size:11px;line-height:1.5;max-height:70vh;overflow-y:auto;white-space:pre-wrap;word-break:break-word;">
        ${escapeHtml(preview)}
        ${content.length > 2000 ? `\n\n… (${(content.length / 1024).toFixed(1)} KB total)` : ''}
      </div>
    `;
    
    body.innerHTML = html;
    toast('✓ Preview loaded', 'lime');
  } catch (err) {
    toast('Failed to preview: ' + err.message, 'red');
  }
};

window.LazNote.askGroqAboutZIP = async function() {
  const session = state.zipSession;
  if (!session || !state.settings.groqKey) {
    toast('Connect Groq in Settings first', 'info');
    return;
  }
  
  const body = document.getElementById('zip-body');
  const typeList = Object.entries(session.typeMap).map(([t, c]) => `${t}: ${c}`).join(', ');
  
  let html = `
    <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;">
      <button class="icon-btn" onclick="LazNote.renderZIPInspector()" style="font-size:14px;">←</button>
      <div style="flex:1;font-weight:600;">AI Analysis</div>
    </div>
    <div style="padding:12px;background:var(--line-2);border-radius:6px;text-align:center;color:var(--ink-60);font-size:13px;">
  ...
  try {
    const prompt = `Analyze this ZIP file structure and provide a brief summary (2-3 sentences):
Name: ${session.name}
Files: ${session.fileCount}
Types: ${typeList}
Key files present: ${session.files.slice(0, 10).map(f => f.path).join(', ')}

What is this project about? What kind of app/tool is it?`;
    ...
    html = `
      ...
      <div style="padding:12px;background:var(--line-2);border-radius:6px;line-height:1.6;font-size:13px;color:var(--ink-80);">
        ${escapeHtml(analysis)}
      </div>
    `;
    
    body.innerHTML = html;
    toast('✓ Analysis complete', 'lime');
  } catch (err) {
    toast('Analysis failed: ' + err.message, 'red');
    console.error('Groq error:', err);
  }
};

window.LazNote.exportZIPInventory = function() {
  const session = state.zipSession;
  if (!session) return;
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `${session.name.replace(/\.zip$/, '')}-inventory.json`;
  a.click();
  URL.revokeObjectURL(url);
  
  toast('✓ Inventory exported', 'lime');
};

window.LazNote.downloadZipSession = async function() {
  const session = state.zipSession;
  if (!session || !session.zip) {
    toast('No ZIP session loaded', 'info');
    return;
  }
  
  try {
    toast('⏳ Packaging ZIP…', 'info');
    const blob = await session.zip.generateAsync({ type: 'blob' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    ...
    toast('✓ ZIP downloaded', 'lime');
  } catch (err) {
    toast('Download failed: ' + err.message, 'red');
  }
};
